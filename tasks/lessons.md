...
## Selectable video providers - 2026-09-20

- Provider selection must change prompt refinement as well as submission. Veo receives direction and images but no motion guide, so refined prompts must describe movement explicitly. Keep first/last-frame inputs distinct from appearance references when validating capabilities: reference-image mode requires 8 seconds, while 720p frame interpolation supports 4, 6, or 8. See `server/promptRefinement.ts`, `server/veo.ts`, and `src/core/videoModels.ts`.
