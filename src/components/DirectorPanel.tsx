...
import { Check, CornerDownLeft, LoaderCircle, MapPin, Mic, MicOff, Minus, Play, RefreshCw, Send, Sparkles, Trash2, Undo2, X } from 'lucide-react';
...
export default function DirectorPanel({ open, active, onOpen, onClose }: { open: boolean; active: boolean; onOpen: () => void; onClose: () => void }) {
  ...
  useEffect(() => { session.setSpeechEnabled(Boolean(capabilities?.elevenLabsTts)); }, [capabilities?.elevenLabsTts, session]);
  ...
  const voiceOn = state.voice !== 'off', speechLocked = state.speech !== 'idle';
  ...
  if (!open) return voiceOn ? <div className="director-dock" role="region" aria-label="Director voice controls"><button onClick={onOpen}><span className={`director-voice-dot ${state.voice}`} /><Mic size={16} /><span>Director · {speechLocked ? state.speech : state.voice}</span></button><button aria-label="Stop director voice" onClick={session.stopVoice}><MicOff size={17} /></button></div> : null;
  return <aside className="assistant-panel director-panel" aria-label="Director">
    <div className="panel-heading"><div><span className="eyebrow"><Sparkles size={12} />{editor.project.demo ? 'Demo cache' : 'Gemini'}</span><h2>Director</h2></div><div className="director-heading-actions"><button className="icon-button" aria-label="Clear director chat" title="Clear conversation and pending proposal" disabled={state.busy || (!state.messages.length && !proposal && !execution && !state.error && !voiceOn)} onClick={() => { setText(''); session.clear(); input.current?.focus(); }}><Trash2 size={16} /></button><button className="icon-button" aria-label="Minimize director" onClick={onClose}><Minus size={18} /></button><button className="icon-button" aria-label="Close director" onClick={() => { session.stopVoice(); studio.patch({ preview: null, directorPicking: false }); onClose(); }}><X size={19} /></button></div></div>
    <div className="director-context"><button className={editor.directorPicking ? 'is-picking' : ''} onClick={pick} disabled={editor.exporting || editor.phoneControl}><MapPin size={14} />{editor.directorPicking ? 'Click the floor...' : editor.directorPlacement ? 'Placement point set' : 'Pick placement point'}</button>{editor.directorPlacement && <button aria-label="Clear placement point" onClick={() => studio.patch({ directorPlacement: null, directorPicking: false })}><X size={13} /></button>}</div>
    <div className="director-messages" ref={messages} role="log" aria-label="Director conversation" aria-live="polite">
      ...
      {state.speech === 'preparing' && <div className="director-thinking" role="status"><LoaderCircle size={15} /> Preparing Director's reply...</div>}
      {state.speech === 'speaking' && <div className="director-thinking" role="status"><Play size={15} /> Director is speaking...</div>}
    </div>
    {pending && <section className="director-proposal" aria-label="Director proposal"><div className="director-proposal-title"><span>{working ? <LoaderCircle size={15} /> : <Sparkles size={15} />}{working ? 'Preparing your changes' : stale ? 'Scene changed · review needed' : 'Ready for your approval'}</span><small>{proposal.actions.length} {proposal.actions.length === 1 ? 'action' : 'actions'}</small></div><p>{proposal.summary}</p>
      ...
      <div className="director-proposal-actions">
        {!working && <button className="button primary" disabled={state.busy || speechLocked || studio.directorBusy()} onClick={() => void session.decide(stale ? 'refresh' : 'approve')}>{stale ? <RefreshCw size={14} /> : <Check size={14} />}{stale ? 'Refresh & review' : execution?.status === 'failed' ? 'Retry preparation' : 'Apply'}</button>}
        {canPreview && <button className="button secondary" disabled={state.busy || speechLocked} onClick={() => editor.preview ? studio.patch({ preview: null }) : session.preview()}><Play size={13} />{editor.preview ? 'End preview' : 'Preview'}</button>}
        {!working && <button className="button secondary" disabled={state.busy || speechLocked} onClick={() => { input.current?.focus(); setText('Change the plan: '); }}>Revise</button>}
        <button className="button secondary" disabled={state.busy || speechLocked} onClick={() => void session.decide('cancel')}>Cancel</button>
      </div>
    </section>}
    ...
    {capabilities && !capabilities.director && <p className="director-config" role="status">{editor.project.demo ? 'Cached demo commands are ready. Unmatched requests and voice need a Gemini connection.' : 'Director needs a Gemini connection. Set GEMINI_API_KEY on the local server, then restart it.'}</p>}
    {capabilities?.director && !capabilities.elevenLabsTts && <p className="director-config" role="status">Set ELEVENLABS_API_KEY and ELEVENLABS_VOICE_ID on the local server to enable spoken Director replies.</p>}
    <form className="director-composer" onSubmit={event => { event.preventDefault(); void send(); }}>
      <textarea ref={input} aria-label="Message director" placeholder={speechLocked ? 'Wait for Director to finish speaking...' : 'Tell the director what you have in mind...'} value={text} maxLength={4000} onChange={event => setText(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); void send(); } }} disabled={state.busy || speechLocked} rows={2} />
      <div className="director-composer-actions"><button type="button" className={`director-mic ${voiceOn ? 'is-on' : ''}`} aria-label={voiceOn ? 'Stop director voice' : 'Start director voice'} aria-pressed={voiceOn} disabled={!voiceOn && (speechLocked || capabilities?.directorVoice === false)} onClick={() => voiceOn ? session.stopVoice() : void session.startVoice()}>{voiceOn ? <MicOff size={17} /> : <Mic size={17} />}<span>{voiceOn ? speechLocked ? state.speech : state.voice : 'Talk to Director'}</span></button><button type="submit" className="director-send" aria-label="Send to director" disabled={!text.trim() || state.busy || speechLocked || capabilities?.director === false && !editor.project.demo}><Send size={16} /></button></div>
    </form>
  </aside>;
}
