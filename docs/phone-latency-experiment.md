...
On 2026-09-19, the companion compiled and signed with Xcode 26.4 and was installed on the paired iPhone 17 Pro Max. The local code signature passed verification. iOS denied automatic launch with a signing/trust error; Developer Mode was enabled. Check the installed developer profile in Settings → General → VPN & Device Management and then open Composition Camera. Playback and hardware results remain **pending** until the phone can launch and export a run.
