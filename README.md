...
- Click **Velocity** in the keyframe timeline for a single velocity lane. **Add key** (or K) creates an independent speed key at the playhead. Drag it horizontally to retime and vertically to change speed, or select it to enter exact **Time** and **Speed** values. Delete and undo work like ordinary keys. There are no easing presets. Speeds are relative weights, blended linearly, that redistribute the whole object's or block's motion within its existing duration. Zero-speed sections pause motion; all-zero keys hold the starting pose. The original pose keys remain editable by switching back.
