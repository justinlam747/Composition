...
export class ImageJobs {
  ...
  async create(projectId: string, input: ImageRequest) {
    const { id, prompt, guideAssetId, count = 1, paired = false, referenceAssetIds = [] } = input;
    const referenceSignature = JSON.stringify(referenceAssetIds);
    ...
    try {
      try {
        ...
        if (existing.projectId !== projectId || existing.prompt !== prompt || existing.guideAssetId !== guideAssetId || (existing.count ?? 1) !== count || !!existing.paired !== paired || JSON.stringify(existing.referenceAssetIds ?? []) !== referenceSignature) throw new AppError(409, 'ID_CONFLICT', 'This ID belongs to a different image request.');
        return existing;
      ...
      if (saved?.id !== id || saved.prompt !== prompt || saved.guideAssetId !== guideAssetId || (saved.count ?? 1) !== count || !!saved.paired !== paired || JSON.stringify(saved.referenceAssetIds ?? []) !== referenceSignature) throw new AppError(400, 'IMAGE_REQUEST_NOT_SAVED', 'Save this image request with your project before generating.');
      ...
      await Promise.all(referenceAssetIds.map(id => this.store.requireAsset(id, 'reference')));
