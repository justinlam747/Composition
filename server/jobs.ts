...
export class Jobs {
  ...
  async recover() {
    for (const job of await this.store.list<StoredJob>('jobs')) {
      if (job.status === 'preparing' && !job.requestId) {
        await this.store.put('jobs', job.id, { ...job, status: 'failed', error: 'The server stopped during submission. Check your provider account before starting another request; it may already have been accepted.' });
      }
    }
  }
  async create(id: string, project: Project, prompt: string) {
    const veo = project.generation?.videoModel === 'veo';
    ...
    try {
      ...
      if (veo && !this.providers.configured.gemini) throw new AppError(503, 'GEMINI_NOT_CONFIGURED', 'Configure GEMINI_API_KEY on the server to generate with Veo.');
      if (!veo && !this.providers.configured.fal) throw new AppError(503, 'FAL_NOT_CONFIGURED', 'Live video is unavailable: configure FAL_KEY on the server.');
      ...
      if (!veo && (!Number.isInteger(project.duration) || project.duration < 4 || project.duration > 10)) throw new AppError(400, 'VIDEO_DURATION', 'Seedance output needs a whole duration of 4–10 seconds. Set the timeline and export again.');
      ...
      if (veo) {
        const issue = veoInputError(project.duration, references, baselineAssetId, lastBaselineAssetId);
      ...
      const job: StoredJob = { id, projectId: project.id, status: 'preparing', mode: 'live', guideAssetId: guide.id, referenceAssetIds: references, prompt,
        duration: project.duration, resolution: '720p', createdAt: new Date().toISOString(), model: veo ? 'veo-3.1-generate-preview' : this.providers.model, sourceSignature: sceneSignature(project), fingerprint, ...(baselineAssetId ? { baselineAssetId } : {}), ...(lastBaselineAssetId ? { lastBaselineAssetId } : {}), ...(project.generation?.excludedReferenceAssetIds?.length ? { excludedReferenceAssetIds: project.generation.excludedReferenceAssetIds } : {}) };
  ...
  private async submit(job: StoredJob) {
    ...
    catch (error) { job.status = 'failed'; job.error = this.message(error, 'Video submission failed. Check provider credentials, model access and quota. If the connection failed during submission, check your provider account before retrying.'); }
  ...
  async refresh(id: string) {
    ...
    try {
      ...
      if (update.status === 'completed' && !job.outputAssetId) throw new AppError(502, 'NO_VIDEO', 'Video generation completed without a video.');
      ...
      job.error = this.message(error, 'Could not retrieve video output. Check model access and the provider job; status checks will retry without submitting again.');
