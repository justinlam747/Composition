...
export interface Providers {
  ...
  model: string;
  image(prompt: string, sourceAssetId?: string): Promise<Asset>;
...
export function liveProviders(store: FileStore): Providers {
  ...
  async function gemini(prompt: string, kind: ProposalKind | 'image', source?: { data: string; mimeType: string }): Promise<{ text: string; image?: { data: string; mimeType: string } }> {
    ...
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(selectedModel)}:generateContent`, {
      ...
      body: JSON.stringify({ contents: [{ role: 'user', parts: [{ text: prompt }, ...(source ? [{ inlineData: source }] : [])] }], generationConfig: image ? { responseModalities: ['TEXT', 'IMAGE'], ...(source ? { imageConfig: { aspectRatio: '16:9' } } : {}) } : { responseMimeType: 'application/json', responseJsonSchema: z.toJSONSchema(proposalContentSchema.options.find(schema => schema.shape.kind.value === kind)!), temperature: .4 } }),
    });
  ...
  async function generateImage(prompt: string, sourceAssetId?: string) {
    ...
    const result = await gemini(prompt, 'image', source);
  ...
  return {
    ...
    async submit(job) {
      ...
      const supporting = images.map((_, i) => `@Image${i + 1}`).slice(job.baselineAssetId ? 1 : 0);
      const result = await fal.queue.submit(job.model, { input: { prompt: `Follow the animation, staging and camera framing in @Video1. ${appearance}${supporting.length ? 'Use ' + supporting.join(', ') + ' as supporting object appearance references. ' : ''}${job.prompt}`, video_urls: [guide], image_urls: images, duration: String(job.duration), resolution: '720p', aspect_ratio: '16:9', generate_audio: false } });
