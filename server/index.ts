...
import { attachDirectorLive } from './directorLive';
...
const closeDirectorLive = attachDirectorLive(server);
let checking = false;
...
timer.unref();
function stop() { clearInterval(timer); closeDirectorLive(); server.close(); void phone?.close(); }
